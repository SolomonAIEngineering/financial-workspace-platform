...
describe('FetchError', () => {
  ...
  describe('status codes', () => {
    ...
    it.each(statusCodes['4xx'])(
      'should handle %i client error',
      (statusCode) => {
        const error = new FetchError({
    ...
    it.each(statusCodes['5xx'])(
      'should handle %i server error',
      (statusCode) => {
        const error = new FetchError({
          message: 'Server error',
          retry: true,
  ...
  describe('context handling', () => {
    ...
    it('should handle various header combinations', () => {
      ...
      expect((error.context?.headers as Headers)?.['Content-Type']).toBe(
        'application/json',
      )
      expect((error.context?.headers as Headers)?.['Authorization']).toBe(
        'Bearer token',
      )
    })
  })
