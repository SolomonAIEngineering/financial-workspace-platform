    ...
    <Menu as="div" className="relative inline-block text-left">
      <Menu.Button className="-m-1.5 flex items-center p-1.5">
        ...
        <span className="hidden lg:flex lg:items-center">
          <span
            className="text-secondary ml-4 text-sm font-semibold leading-6"
            aria-hidden="true"
          >
            {user.username}
          </span>
          <ChevronDownIcon
            className="text-secondary ml-2 h-5 w-5"
            aria-hidden="true"
          />
        </span>
      </Menu.Button>
      ...
      >
        <Menu.Items className="bg-secondary absolute right-0 z-10 mt-2 w-32 origin-top-right rounded-md py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
            ...
            <Menu.Item key={item.name}>
              {({ active }) => (
                <a
                  ...
                  className={cn(
                    active ? 'bg-gray-50' : '',
                    'text-secondary block px-3 py-1 text-sm leading-6',
                  )}
                >
                  {item.name}
                </a>
