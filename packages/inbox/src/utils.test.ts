...
describe('getInboxIdFromEmail', () => {
  ...
  test('extracts inbox id from staging email', () => {
    expect(getInboxIdFromEmail('test456@inbox.staging.solomon-ai.app')).toBe(
      'test456',
    )
  })

  test('handles email with special characters', () => {
    expect(getInboxIdFromEmail('user.name-123@inbox.solomon-ai.app')).toBe(
      'user.name-123',
    )
  })
...
describe('getInboxEmail', () => {
  ...
  test('generates staging email in non-production environment', () => {
    ...
    expect(getInboxEmail('user123')).toBe(
      'user123@inbox.staging.solomon-ai.app',
    )
  })
