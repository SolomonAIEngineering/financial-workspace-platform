...
import { BankConnectionStatus } from '@solomonai/prisma/index';
import { addDays } from 'date-fns';
import { addNewUser } from '@solomonai/jobs/src/trigger';
 ...
 */
export const connectionExpirationJob: any = schemaTask({
  ...
  run: async (payload) => {
    ...
    try {
      ...
      for (const connection of warningConnections) {
        ...
        // TODO: Check if notification was already sent recently to prevent duplicates
        await tasks.trigger<typeof addNewUser>("add-new-user", {
          name: "Example user",
