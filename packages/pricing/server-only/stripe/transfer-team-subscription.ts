...
export const transferTeamSubscription = async ({
  ...
  const teamCustomerId = team.stripeCustomerId
  ...
  if (team.subscription) {
    teamSubscription = await stripe.subscriptions.retrieve(
      team.subscription.planId,
    )
  ...
  await stripe.customers.update(teamCustomerId, {
    name: user.name ?? team.name ?? undefined,
    email: user.email ?? team.email ?? undefined,
  })
  ...
  if (teamSubscriptionRequired && !teamSubscription) {
    const numberOfSeats = await prisma.usersOnTeam.count({
      where: {
        teamId: team.id,
      },
    })
