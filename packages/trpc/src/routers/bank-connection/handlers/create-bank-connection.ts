import { AccountType, BankConnectionStatus, Prisma, prisma } from '@solomonai/prisma';
import { CreateBankConnectionResponseSchema, CreateBankConnectionSchema } from '../schema';
 ...
 */
function mapAccountType(type: 'credit' | 'depository' | 'other_asset' | 'loan' | 'other_liability'): AccountType {
...
export const createBankConnection = protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
        try {
            ...
            // Validate input data
            validateBankConnectionInput(teamId, userId, provider, itemId, accessToken, accounts);
