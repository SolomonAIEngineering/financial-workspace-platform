...
const extractTeamId = (input: unknown): string => {
  try {
    ...
    if (error instanceof z.ZodError) {
      const errorMessage = error.errors.map((err: z.ZodIssue) => err.message).join(', ');
      throw new TRPCError({
        code: 'BAD_REQUEST',
 ...
 * ```
 */
const verifyRole = (teamMembership: { role: TeamRole }, requiredRole: TeamRole) => {
  if (teamMembership.role !== requiredRole) {
    const roleMessage = requiredRole === TeamRole.OWNER
 ...
 * ```
 */
const createEnhancedContext = (ctx: TrpcContext, membershipInfo: TeamMembershipInfo) => {
  return {
    ...ctx,
