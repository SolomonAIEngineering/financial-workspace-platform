...
const lineItemSchema = z.object({
  description: z
    .string()
    .describe('A brief description of the purchased item or service.'),
  quantity: z.number().describe('The quantity of the item purchased.'),
  unitPrice: z.number().describe('The price per unit of the item.'),
  amount: z
    .number()
    .describe('The total amount for this line item (quantity * unitPrice).'),
})
...
const schema = z.object({
  ...
  purchaseOrderNumber: z
    .string()
    .optional()
    .describe('The associated purchase order number, if any.'),
  paymentTerm: z
    .string()
    .optional()
    .describe('Payment terms of the invoice (e.g., Net 30).'),
  ...
  vendorAddress: z
    .string()
    .optional()
    .describe('The complete address of the vendor.'),
  ...
  website: z
    ...
    .optional()
    .describe(
      "The vendor's website domain without protocol (e.g. example.com).",
    ),
  ...
  customerName: z
    .string()
    .optional()
    .describe("The customer's name or company name."),
  ...
  currency: z
    .string()
    .describe('The currency code of the invoice (e.g., USD, EUR).'),
  subtotal: z
    .number()
    .optional()
    .describe('The subtotal amount before taxes and additional fees.'),
  tax: z.number().optional().describe('The total tax amount on the invoice.'),
  tip: z.number().optional().describe('Any included tip amount.'),
  previousUnpaidBalance: z
    .number()
    .optional()
    .describe('Any outstanding balance prior to this invoice.'),
  invoiceTotal: z
    .number()
    .optional()
  ...
  amountDue: z
    .number()
    .optional()
    .describe('The amount still due (outstanding) on the invoice.'),

  // Line Items
  lineItems: z
    .array(lineItemSchema)
    .optional()
    .describe('List of items/services included in the invoice.'),

  // Additional fields
  type: z
    .string()
    .optional()
    .describe('The type of the document, e.g., invoice.'),
  status: z.string().optional().describe('The status of the invoice.'),
  billingPeriod: z
    .string()
    .optional()
    .describe('The billing period for the invoice.'),
  notes: z
    .string()
    .optional()
    .describe('Any additional notes related to the invoice.'),
  terms: z.string().optional().describe('Terms associated with the invoice.'),
  discounts: z
    .number()
    .optional()
    .describe('Any discounts applied to the invoice.'),
  shipping: z
    .number()
    .optional()
    .describe('Shipping costs associated with the invoice.'),
  handling: z
    .number()
    .optional()
    .describe('Handling fees associated with the invoice.'),
  taxRate: z
    .number()
    .optional()
    .describe('The tax rate applied to the invoice.'),
  taxId: z
    .string()
    .optional()
    .describe('The tax identification number for the vendor.'),
  bankDetails: z.string().optional().describe('Bank details for payment.'),
  paymentMethod: z
    .string()
    .optional()
    .describe('The method of payment for the invoice.'),
  reference: z
    .string()
    .optional()
    .describe('Any reference number associated with the invoice.'),
})
