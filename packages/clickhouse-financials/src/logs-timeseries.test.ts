...
describe.each([10, 100, 1_000, 10_000])('with %i requests', (n) => {
  test(
    ...
    async (t) => {
      ...
      for (let i = 0; i < requests.length; i += batchSize) {
        await ch.api.insert(requests.slice(i, i + batchSize))
      }

      // Wait for data to be fully inserted
    ...
    { timeout: 180_000 }, // Increased timeout for larger datasets
  )
})
