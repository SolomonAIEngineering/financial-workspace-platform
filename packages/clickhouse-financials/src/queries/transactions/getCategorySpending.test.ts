  ...
  for (let month = 0; month < 6; month++) {
    ...
    for (const category of categories) {
      ...
      const transactionCount =
        Math.floor(Math.random() * 10 * categoryWeights[category]) + 1
        ...
        if (result.err) {
          ...
          if (result.val && result.val.length > 0) {
            ...
            for (const categoryData of result.val) {
              ...
              const totalPercentage = result.val.reduce(
            ...
            for (let i = 0; i < result.val.length - 1; i++) {
              expect(result.val[i].total_expenses).toBeGreaterThanOrEqual(
                result.val[i + 1].total_expenses,
              )
            }
          } else {
