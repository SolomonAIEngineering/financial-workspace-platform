  ...
  for (let month = 0; month < 6; month++) {
    ...
    for (const merchant of merchants) {
      ...
      for (let i = 0; i < transactionCount; i++) {
        ...
        const amount =
          (-1 * Math.round((baseAmount + Math.random() * baseAmount) * 100)) /
          100 // Negative for spending

        // Assign a consistent category for each merchant
        const category =
          categories[merchants.indexOf(merchant) % categories.length]
      ...
      try {
        ...
        try {
          ...
          console.info(
            ...
            tableExists.val?.map((table) => table.name),
          )
        ...
        if (!insertionSuccessful) {
          ...
          throw new Error(
            'Test insertion failed, cannot proceed with batch insertion',
          )
        } else {
          ...
          try {
            ...
            if (batchResult.err) {
              ...
              throw new Error(
                'Batch insertion failed: ' + batchResult.err.message,
              )
            }
          } catch (error) {
        ...
        console.info('Creating merchant spending materialized view...')
        try {
        ...
        console.info('Testing getMerchantSpending function...')
        ...
        console.info('getMerchantSpending result:', result)
        ...
        if (result.err) {
          ...
          if (result.val && result.val.length > 0) {
            ...
            for (const merchantData of result.val) {
              ...
              expect(
                new Date(merchantData.first_transaction).toString(),
              ).not.toBe('Invalid Date')
              expect(
                new Date(merchantData.latest_transaction).toString(),
              ).not.toBe('Invalid Date')

              // Verify that first_transaction is before or equal to latest_transaction
              expect(
                new Date(merchantData.first_transaction) <=
                  new Date(merchantData.latest_transaction),
              ).toBe(true)
            }
          } else {
