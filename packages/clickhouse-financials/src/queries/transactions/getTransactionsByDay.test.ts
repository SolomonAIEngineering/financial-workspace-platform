        ...
        if (result.err) {
          ...
          if (result.val && result.val.length > 0) {
            ...
            for (const dayData of result.val!) {
              ...
              expect(dayData.total_amount).toBeCloseTo(
                dayData.total_income - dayData.total_expenses,
                2,
              )

              // Verify that average_amount is calculated correctly
              expect(dayData.average_amount).toBeCloseTo(
                dayData.total_amount / dayData.transaction_count,
                2,
              )
            }
          } else {
