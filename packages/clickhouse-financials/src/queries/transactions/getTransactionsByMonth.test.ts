        ...
        try {
          ...
          try {
            ...
            if (simpleResult.err) {
              ...
              throw new Error(
                'Could not insert simple data: ' + simpleResult.err.message,
              )
            }
        ...
        if (!insertionSuccessful) {
          ...
          try {
            ...
            if (batchResult.err) {
              ...
              throw new Error(
                'Could not insert batch data: ' + batchResult.err.message,
              )
            }
          } catch (error) {
            ...
            for (const transaction of transactions) {
              try {
                ...
                if (singleResult.err) {
                  ...
                  successCount++
                  console.info(
                    `Inserted transaction ${transaction.id} (${successCount}/${transactions.length})`,
                  )
                }
              } catch (error) {
            ...
            console.info(
              `Successfully inserted ${successCount}/${transactions.length} transactions one by one`,
            )
        ...
        if (result.err) {
          ...
          if (result.val && result.val.length > 0) {
            ...
            for (const monthData of result.val) {
              ...
              expect(monthData.total_amount).toBeCloseTo(
                monthData.total_income - monthData.total_expenses,
                2,
              )

              // Verify that average_amount is calculated correctly
              expect(monthData.average_amount).toBeCloseTo(
                monthData.total_amount / monthData.transaction_count,
                2,
              )
            }
          } else {
