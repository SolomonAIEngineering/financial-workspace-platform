...
function generateMonthlyCashFlowForecastData(
    ...
    for (let i = 0; i < n; i++) {
        ...
        forecastData.push({
            ...
            first_transaction_date: new Date(monthStart.getFullYear(), monthStart.getMonth(), Math.floor(Math.random() * 10) + 1).toISOString().replace('T', ' ').substring(0, 19),
            last_transaction_date: new Date(monthStart.getFullYear(), monthStart.getMonth(), Math.floor(Math.random() * 10) + 15).toISOString().replace('T', ' ').substring(0, 19),
            transaction_titles: Array.from({ length: Math.floor(Math.random() * 5) + 1 }, () => {
                const titles = ['Salary', 'Freelance Income', 'Investment Returns', 'Client Payment', 'Rental Income']
        ...
        forecastData.push({
            ...
            first_transaction_date: new Date(monthStart.getFullYear(), monthStart.getMonth(), Math.floor(Math.random() * 10) + 1).toISOString().replace('T', ' ').substring(0, 19),
            last_transaction_date: new Date(monthStart.getFullYear(), monthStart.getMonth(), Math.floor(Math.random() * 10) + 15).toISOString().replace('T', ' ').substring(0, 19),
            transaction_titles: Array.from({ length: Math.floor(Math.random() * 8) + 3 }, () => {
                const titles = ['Rent', 'Utilities', 'Groceries', 'Subscriptions', 'Insurance', 'Car Payment', 'Phone Bill', 'Internet']
                return titles[Math.floor(Math.random() * titles.length)]
            })
        });
    }
        ...
        try {
            ...
            try {
                const result = await getMonthlyCashFlowForecastFn({
                    ...
                    end: new Date(new Date().setMonth(new Date().getMonth() + 6)).getTime(),
                    limit: 100
                })
                    ...
                    if (result.val) {
                        ...
                        result.val.forEach(forecast => {
                            if (lastMonth === forecast.month_start) {
                                expect(forecast.is_expense).toBeGreaterThanOrEqual(lastIsExpense)
                            } else if (lastMonth !== '') {
