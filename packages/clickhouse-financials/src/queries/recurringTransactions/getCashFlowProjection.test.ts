...
function generateRecurringTransactionData(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const daysToNextExecution = Math.floor(
      ...
      try {
        ...
        if (result.err) {
          ...
          if (result.val && result.val.length > 0) {
            ...
            expect(projection.net_projected_balance).toBe(
              projection.projected_income - projection.projected_expenses,
            )
          ...
          if (!result60Days.err && result60Days.val && result.val) {
            ...
            if (result60Days.val.length > 0 && result.val.length > 0) {
              expect(
                result60Days.val[0].total_recurring_transactions,
              ).toBeGreaterThanOrEqual(
                result.val[0].total_recurring_transactions,
              )
            }
          }
        }
