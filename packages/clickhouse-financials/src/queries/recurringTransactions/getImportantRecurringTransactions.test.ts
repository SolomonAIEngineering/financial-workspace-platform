...
function generateRecurringTransactionData(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const importanceLevel =
      importanceLevels[Math.floor(Math.random() * importanceLevels.length)]
    ...
    if (status === 'active') {
      ...
      daysToNextExecution = Math.floor(
        (nextScheduledDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
      )
    }
    ...
    recurringTransactions.push({
      ...
      next_scheduled_date: nextScheduledDate
        ? nextScheduledDate.toISOString().replace('T', ' ').substring(0, 19)
        : null,
...
describe('getImportantRecurringTransactions', () => {
  test('accurately retrieves important recurring transactions', async (t) => {
    ...
    try {
      ...
      console.info(
        'Generated sample recurring transactions:',
      ...
      const activeTransactions = recurringTransactions.filter(
        (t) =>
          t.status === 'active' &&
      ...
      const getImportantRecurringTransactionsFn =
        getImportantRecurringTransactions(ch.querier)
      ...
      try {
        ...
        if (result.err) {
          console.error(
            'Error in getImportantRecurringTransactions:',
            result.err,
          )
          ...
          if (result.val) {
            ...
            result.val.forEach((transaction) => {
              if (lastImportance === transaction.importance_level) {
                expect(Math.abs(transaction.amount)).toBeLessThanOrEqual(
                  lastAmount,
                )
              } else {
