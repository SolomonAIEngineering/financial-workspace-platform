        ...
        try {
            ...
            try {
                ...
                if (result.err) {
                    ...
                    throw new Error('Error in getExpensesByCostType: ' + result.err.message);
                } else {
                    // Verify the results
                    ...
                    for (const monthData of result.val!) {
                        ...
                        expect(monthData.total_costs).toBeCloseTo(monthData.fixed_costs + monthData.variable_costs, 1);
                        ...
                        if (monthData.total_costs > 0) {
                            expect(monthData.fixed_cost_percentage).toBeCloseTo(
                                (monthData.fixed_costs / monthData.total_costs) * 100, 1
                            );

                            expect(monthData.variable_cost_percentage).toBeCloseTo(
                                (monthData.variable_costs / monthData.total_costs) * 100, 1
                            );

                            // Check that percentages add up to 100%
                            expect(monthData.fixed_cost_percentage + monthData.variable_cost_percentage).toBeCloseTo(100, 1);
                        }
                    }
