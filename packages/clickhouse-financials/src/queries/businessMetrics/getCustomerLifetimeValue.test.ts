    ...
    try {
      ...
      try {
        ...
        if (result.err) {
          ...
          throw new Error(
            'Error in getCustomerLifetimeValue: ' + result.err.message,
          )
        } else {
          // Verify the results
          ...
          if (result.val && result.val.length > 0) {
            ...
            if (
              clvMetrics.avg_customer_lifetime_months !== null &&
              clvMetrics.monthly_churn_rate !== null &&
              clvMetrics.monthly_churn_rate > 0
            ) {
              // Should be approximately 1 / churn_rate
              expect(clvMetrics.avg_customer_lifetime_months).toBeCloseTo(
                1 / clvMetrics.monthly_churn_rate,
                1,
              )
            }
            ...
            if (
              clvMetrics.customer_lifetime_value !== null &&
              clvMetrics.arpu !== null &&
              clvMetrics.avg_customer_lifetime_months !== null
            ) {
              // Should be ARPU * avg_customer_lifetime_months
              expect(clvMetrics.customer_lifetime_value).toBeCloseTo(
                clvMetrics.arpu * clvMetrics.avg_customer_lifetime_months,
                1,
              )
            }
          }
