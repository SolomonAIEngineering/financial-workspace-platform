...
describe('getCashRunway', () => {
  test('accurately calculates cash runway', async (t) => {
    ...
    try {
      ...
      console.info(
        'Testing getCashRunway function with calculated burn rate...',
      )
      ...
      try {
        ...
        if (result.err) {
          ...
          if (result.val && result.val.length > 0) {
            ...
            expect(runway.runway_months).toBeCloseTo(
              currentCash / runway.burn_rate,
              1,
            )
            ...
            console.info(
              'Testing getCashRunway function with custom burn rate...',
            )
            ...
            console.info(
              'getCashRunway result (custom burn rate):',
              customResult,
            )

            if (customResult.err) {
              console.error(
                'Error in getCashRunway with custom burn rate:',
                customResult.err,
              )
              ...
              expect(customRunway.runway_months).toBeCloseTo(
                currentCash / customBurnRate,
                1,
              )
