...
import { Ok } from '@solomonai/error';
import {
    insertRecurringTransaction
} from './insertRecurringTransaction';
import {
    insertRecurringTransactions
} from './insertRecurringTransactions';
import { z } from 'zod';
    ...
    describe('Basic Functionality', () => {
        test(
            ...
            async (t) => {
                ...
                for (const transaction of testTransactions) {
                    const validationResult = recurringTransactionSchema.safeParse(transaction);
                ...
                const result = await insertFn(testTransactions as unknown as RecurringTransaction[]);
        ...
        test(
            ...
            async (t) => {
                ...
                // Validate test data against schema
                const validationResult = recurringTransactionSchema.safeParse(testTransaction);
                if (!validationResult.success) {
                    console.error('Schema validation failed:', validationResult.error.format());
                    throw new Error('Schema validation failed');
                }

                const result = await insertFn(testTransaction as unknown as RecurringTransaction);
                console.log('Insert result:', result);
                expect(result.success).toBe(true);
