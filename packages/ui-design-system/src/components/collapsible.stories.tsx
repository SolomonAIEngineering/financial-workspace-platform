            ...
            >
                ...
                <CollapsibleContent className="space-y-2">
                    <div className="rounded-md border px-4 py-3 text-sm">
                        A Collapsible component allows users to toggle the visibility of content,
                        providing a way to hide information when it's not needed and reveal it
                        when it is. It's often used for FAQs, navigation menus, or any section
                        that needs to be expandable and collapsible.
                    </div>
                </CollapsibleContent>
            </Collapsible>
        );
    ...
    render: () => {
        ...
        return (
            <Collapsible
                ...
                className="w-[350px] space-y-2"
            >
                <div className="flex items-center justify-between space-x-4 px-4 border rounded-t-lg py-2">
                    <h4 className="text-sm font-semibold">
                        Animated Collapsible
                    </h4>
                    <CollapsibleTrigger asChild>
                        <Button variant="ghost" size="sm" className="transition-transform duration-200">
                            <CaretSortIcon className={`h-4 w-4 ${isOpen ? 'rotate-180' : ''}`} />
                            <span className="sr-only">Toggle</span>
                        </Button>
                    </CollapsibleTrigger>
                </div>
