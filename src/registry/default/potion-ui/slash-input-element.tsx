...
export function SlashInputElement({
  ...
  React.useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      scrollContainer.classList.toggle('show-top-shadow', !isAtTop);
      scrollContainer.classList.toggle('show-bottom-shadow', !isAtBottom);
    };
  ...
  return (
    <PlateElement as="span" data-slate-value={element.value} {...props}>
      <InlineCombobox element={element} trigger="/">
        ...
        <InlineComboboxContent
          variant="slash"
          className="scrollable-content overflow-hidden rounded-lg border border-solid border-gray-200 p-1 shadow-md"
        >
          <div
            ref={scrollContainerRef}
            className="custom-scrollbar relative max-h-[300px] overflow-y-auto"
          >
            <InlineComboboxEmpty>No results</InlineComboboxEmpty>

            {groups.map(({ group, items }) => (
              <InlineComboboxGroup key={group}>
                <InlineComboboxGroupLabel className="sticky top-0 z-10 bg-white px-3 py-1 text-xs font-medium tracking-wider text-gray-500 uppercase">
                  {group}
                </InlineComboboxGroupLabel>
                {items.map(
                  ({
                    ...
                    <InlineComboboxItem
                      key={value}
                      className="mx-1 rounded-md px-3 py-2 transition-colors hover:bg-gray-100"
                      value={value}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="flex size-10 items-center justify-center rounded-md border border-gray-200 bg-white text-gray-600">
                          {icon}
                        </div>
          ...
          <div className="sticky bottom-0 mt-1 flex items-center justify-between border-t border-gray-200 bg-white px-3 py-2 text-xs text-gray-500">
