...
describe('Quota System', () => {
  ...
  describe('checkQuotaLimit', () => {
    ...
    test('should throw error when specified', () => {
      expect(() => checkQuotaLimit(15, 10, { throwError: true })).toThrow(
        'Quota exceeded: usage 15 exceeds limit 10',
      )
    })
  ...
  describe('Quota Progression', () => {
    test('limits should increase from free to pro tier', () => {
      ...
      compareNumericLimits(
        QUOTA.free.maxTransactions,
        QUOTA.pro.maxTransactions,
      )
      ...
      Object.entries(usage).forEach(([key, value]) => {
        if (typeof value === 'number') {
          ...
          Object.entries(value as Record<string, number>).forEach(
            ([subKey, subValue]) => {
              const category = QUOTA.free[key as keyof QuotaLimits] as Record<
                string,
                number
              >
  ...
  describe('Quota Validation', () => {
    ...
    test('should have consistent storage units', () => {
      ;(['free', 'pro', 'enterprise'] as const).forEach((tierName) => {
        ...
        if (
          tier.storage.maxFileSize !== -1 &&
          tier.storage.maxTotalStorage !== -1
        ) {
          expect(tier.storage.maxFileSize).toBeLessThanOrEqual(
            tier.storage.maxTotalStorage,
          )
        }
      })
    })
  ...
  describe('Feature Limits', () => {
    describe('Invoicing Limits', () => {
      test('should have reasonable template limits', () => {
        expect(QUOTA.free.invoicing.maxTemplates).toBeLessThan(
          QUOTA.pro.invoicing.maxTemplates,
        )
