...
test('rejects invalid version', () => {
  ...
  expect(() => KeyV1.fromString(invalidKey)).toThrow(
    'Only version 1 keys are supported',
  )
})

test('validates byteLength boundaries', () => {
  // Test minimum length
  expect(() => new KeyV1({ byteLength: 7 })).toThrow(
    'v1 keys must be between 8 and 255 bytes long',
  )
  expect(() => new KeyV1({ byteLength: 8 })).not.toThrow()

  // Test maximum length
  expect(() => new KeyV1({ byteLength: 255 })).not.toThrow()
  expect(() => new KeyV1({ byteLength: 256 })).toThrow(
    'v1 keys must be between 8 and 255 bytes long',
  )
})
