        ...
        <div className="relative pb-12 md:pb-20">
          ...
          <div className="text-center">
            ...
            <div className="relative h-32 [mask-image:linear-gradient(0deg,transparent,var(--color-white)_40%,var(--color-white))]">
              <div className="before:bg-linear-to-b pointer-events-none absolute left-1/2 top-0 -z-10 h-[480px] w-[480px] -translate-x-1/2 rounded-full before:absolute before:inset-0 before:-z-20 before:rounded-full before:from-slate-400/20 before:to-transparent before:to-20% after:absolute after:inset-0 after:-z-20 after:m-px after:rounded-full after:bg-slate-900">
                {items.map((item, index) => (
                  <Transition
                    key={index}
                    as="div"
                    show={active === index}
                    className={`data-closed:absolute data-enter:data-closed:-rotate-[60deg] data-leave:data-closed:rotate-[60deg] data-closed:opacity-0 data-enter:duration-700 data-leave:duration-300 absolute inset-0 -z-10 h-full transform transition ease-[cubic-bezier(0.68,-0.3,0.32,1)]`}
                    unmount={false}
                    appear={true}
                  >
                    <Image
              ...
              <div className="relative flex flex-col" ref={testimonials}>
                {items.map((item, index) => (
                  <Transition
                    ...
                    as="div"
                    show={active === index}
                    className={`data-closed:absolute data-enter:data-closed:-translate-x-4 data-leave:data-closed:translate-x-4 data-closed:opacity-0 data-enter:duration-500 data-enter:delay-200 data-leave:duration-300 data-leave:delay-200 transform transition ease-out`}
                    unmount={false}
                    appear={true}
                  >
                    <div className="bg-linear-to-r from-slate-200/60 via-slate-200 to-slate-200/60 bg-clip-text text-xl font-bold text-transparent">
                      {item.quote}
                    </div>
                  </Transition>
                ))}
