...
export default function TransactionNotesDemo() {
  ...
  const [currentTemplate, setCurrentTemplate] = React.useState(
    NOTE_TEMPLATES[0].value
  );
  const [editorValue, setEditorValue] = React.useState<Value>(
    NOTE_TEMPLATES[0].template
  );
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 py-8">
      <h2 className="mb-4 text-2xl font-bold">Transaction Note Editor</h2>
      ...
      <div className="mb-4 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={toggleReadOnly}>
      ...
      <div className="rounded-md border bg-white p-4 dark:bg-gray-800">
        <DndProvider backend={HTML5Backend}>
          <Plate
            ...
            readOnly={isReadOnly}
          >
            <EditorContainer className="min-h-[300px] rounded-md border border-input">
              <Editor
                variant="default"
      ...
      <div className="mt-4 text-sm text-gray-500 dark:text-gray-400">
        <p>
          This editor provides a document-like experience for transaction notes.
        </p>
        <p>
          Changes are tracked and will be saved when you click the "Save Notes"
          button.
        </p>
        <p>
          {isReadOnly
            ? 'The document is currently locked for viewing only.'
            : 'The document is currently editable.'}
        </p>
      </div>
    </div>
  );
