  ...
  comment: {
    ...
    onSelect: ({ editor }) => {
      setTimeout(() => {
        ...
        const plugin = editor.getPlugin(CommentsPlugin);

        if (editor.selection && plugin?.tf?.comment) {
          plugin.tf.comment.setDraft();
          editor.tf.collapse();
          plugin.setOption('activeId', getDraftCommentKey());
          plugin.setOption(
            'commentingBlock',
