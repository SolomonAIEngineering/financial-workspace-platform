 ...
 * 
 * This service provides functions for interacting with the Plaid API.
...
import { type CountryCode, type LinkTokenCreateRequest, type Products, Configuration, PlaidApi, PlaidEnvironments } from 'plaid';
...
if (!PLAID_CLIENT_ID || !PLAID_SECRET) {
    console.warn('Missing Plaid credentials. Set PLAID_CLIENT_ID and PLAID_SECRET environment variables.');
}
...
export const createLinkToken = async (userId: string, redirectUri?: string): Promise<string> => {
    try {
        ...
        throw new Error(`Failed to create link token: ${error instanceof Error ? error.message : String(error)}`);
    }
};
...
export const exchangePublicToken = async (publicToken: string) => {
    try {
        ...
        throw new Error(`Failed to exchange public token: ${error instanceof Error ? error.message : String(error)}`);
    }
};
...
export const getInstitutionById = async (institutionId: string) => {
    try {
        ...
        throw new Error(`Failed to get institution: ${error instanceof Error ? error.message : String(error)}`);
    }
};
    ...
    try {
        ...
        while (hasMore) {
            ...
            const mappedTransactions = data.added.map(transaction => {
                ...
                if (!bankAccountId) {
                    console.warn(`No matching bank account found for transaction ${transaction.transaction_id}`);
        ...
        throw new Error(`Failed to sync transactions: ${error instanceof Error ? error.message : String(error)}`);
    }
};
...
export const getItemDetails = async (accessToken: string) => {
    try {
        ...
        return {
            ...
            status: (response.data.item as any).status || { transactions: { last_successful_update: null } },
        };
    } catch (error) {
        console.error('Error getting item details:', error);

        throw new Error(`Failed to get item details: ${error instanceof Error ? error.message : String(error)}`);
    }
};
 ...
 */
export const updateItemWebhook = async (accessToken: string, webhookUrl: string) => {
    try {
        ...
        throw new Error(`Failed to update item webhook: ${error instanceof Error ? error.message : String(error)}`);
    }
};
