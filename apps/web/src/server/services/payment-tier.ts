 ...
 * 
 * Server-side implementation of payment tier feature gates.
 * This module provides utilities for enforcing feature limits based on user subscription tiers.
 * It integrates with Stripe to determine a user's current subscription level and enforces
 * appropriate limitations on features such as team creation, bank account connections,
 * invoice generation, file uploads, and more.
 * 
...
/**
 * Available subscription tiers within the application.
 * Each tier corresponds to a different level of access and feature availability.
 * 
 * @enum {string}
 */
...
/**
 * Feature limitations for each payment tier.
 * Defines the maximum number of resources a user can access based on their subscription level.
 * 
 ...
 * Users start with the FREE tier unless they explicitly subscribe to a higher tier.
 * 
...
/**
 * Mapping of Stripe price IDs to internal payment tiers.
 * This mapping is used to determine a user's tier based on their active Stripe subscription.
 * 
 ...
 * This function queries the Stripe API to determine the user's active subscription
 * and maps it to the appropriate internal payment tier.
 * 
...
/**
 * Gets the maximum number of teams a user can create based on their payment tier.
 * 
...
/**
 * Gets the maximum number of team members allowed per team based on payment tier.
 * 
 * @param {PaymentTier} tier - The user's payment tier
...
/**
 * Gets the maximum storage (in GB) a user is allowed based on their payment tier.
 * 
 * @param {PaymentTier} tier - The user's payment tier
...
/**
 * Gets the maximum number of reports a user can create based on their payment tier.
 * 
 * @param {PaymentTier} tier - The user's payment tier
...
/**
 * Gets the maximum number of invoices a user can create based on their payment tier.
 * 
 * @param {PaymentTier} tier - The user's payment tier
...
/**
 * Gets the maximum number of bank accounts a user can connect based on their payment tier.
 * 
 * @param {PaymentTier} tier - The user's payment tier
...
/**
 * Gets the maximum file size (in MB) a user can upload based on their payment tier.
 * 
 * @param {PaymentTier} tier - The user's payment tier
...
/**
 * Gets the transaction history retention period (in days) based on payment tier.
 * 
 * @param {PaymentTier} tier - The user's payment tier
...
/**
 * Gets the maximum number of custom transaction categories allowed based on payment tier.
 * 
 * @param {PaymentTier} tier - The user's payment tier
...
/**
 * ResourceValidator class provides methods to validate if a user can create or modify 
 * resources based on their subscription tier.
 * 
 * This class encapsulates all validation logic for enforcing resource limits in a single place,
 * making it easy to check permissions before attempting to create or modify resources.
 * 
 * @class
 * @example
 * ```typescript
...
export class ResourceValidator {
    /**
     * Validates whether a user can create more teams based on their subscription tier.
     * This method throws a TRPCError if the user has reached their team creation limit.
     * 
     * @param {string} userId - The user's unique identifier
     * @throws {TRPCError} If the user has reached their team limit or if validation fails
     */
    public async validateTeamCreation(userId: string): Promise<void> {
        try {
            ...
            console.log(`User ${userId} has created ${userTeamsCount} teams. Max allowed: ${maxTeams} on tier ${tier}`);
    ...
    /**
     * Validates whether a team can add more members based on the owner's subscription tier.
     * This method verifies that adding new members would not exceed the plan's limitations
     * for team size, and throws a TRPCError if the limit would be exceeded.
     * 
     ...
     * @throws {TRPCError} If adding members would exceed the plan limit or if validation fails
     */
    ...
    /**
     * Validates whether a user can create more invoices based on their subscription tier.
     * This method throws a TRPCError if the user has reached their invoice creation limit.
     * 
     * @param {string} userId - The user's unique identifier
     * @throws {TRPCError} If the user has reached their invoice limit or if validation fails
     */
    ...
    /**
     * Validates whether a user can connect more bank accounts based on their subscription tier.
     * This method throws a TRPCError if the user has reached their bank account limit.
     * 
     * @param {string} userId - The user's unique identifier
     * @throws {TRPCError} If the user has reached their bank account limit or if validation fails
     */
    ...
    /**
     * Validates whether a file can be uploaded based on the user's subscription tier.
     * This method throws a TRPCError if the file size exceeds the allowed limit.
     * 
     * @param {string} userId - The user's unique identifier
     * @param {number} fileSizeInBytes - The size of the file to upload in bytes
     * @throws {TRPCError} If the file size exceeds the allowed limit or if validation fails
     */
    public async validateFileUpload(userId: string, fileSizeInBytes: number): Promise<void> {
        try {
            const tier = await getUserTier(userId);
     ...
     * This method throws a TRPCError if the user has reached their category limit.
     * 
     * @param {string} userId - The user's unique identifier
     * @param {string} teamId - The team ID
     * @throws {TRPCError} If the custom category limit is reached or if validation fails
     */
    public async validateCustomCategoryCreation(userId: string, teamId: string): Promise<void> {
        try {
            const tier = await getUserTier(userId);
    ...
    /**
     * Validates whether a user can create more reports based on their subscription tier.
     * This method throws a TRPCError if the user has reached their report creation limit.
     * 
     * @param {string} userId - The user's unique identifier
     * @throws {TRPCError} If the user has reached their report limit or if validation fails
     */
    ...
    /**
     * Validates whether a user can create more documents based on their subscription tier.
     * This method throws a TRPCError if the user has reached their document creation limit.
     * 
     * @param {string} userId - The user's unique identifier
     * @throws {TRPCError} If the user has reached their document limit or if validation fails
     */
    ...
    /**
     * Validates whether a user has sufficient storage remaining based on their subscription tier.
     * This method throws a TRPCError if the user has reached their storage limit.
     * 
     * @param {string} userId - The user's unique identifier
     * @param {number} additionalStorageNeededInBytes - Additional storage needed in bytes
     * @throws {TRPCError} If the storage limit would be exceeded or if validation fails
     */
    public async validateStorageUsage(userId: string, additionalStorageNeededInBytes: number = 0): Promise<void> {
        try {
            const tier = await getUserTier(userId);
            ...
            if (totalStorageUsedBytes + additionalStorageNeededInBytes > maxStorageBytes) {
    ...
    /**
     * Validates whether a user is within their API request limit based on their subscription tier.
     * This method throws a TRPCError if the user has reached their API request limit for the day.
     * 
     * @param {string} userId - The user's unique identifier
     * @throws {TRPCError} If the API request limit has been reached or if validation fails
     */
    ...
    /**
     * Validates whether a user can create more tracker projects based on their subscription tier.
     * This method throws a TRPCError if the user has reached their tracker project limit.
     * 
     * @param {string} userId - The user's unique identifier
     * @throws {TRPCError} If the tracker project limit is reached or if validation fails
     */
    ...
    /**
     * Validates whether a user can install more apps based on their subscription tier.
     * This method throws a TRPCError if the user has reached their app installation limit.
     * 
     * @param {string} userId - The user's unique identifier
     * @throws {TRPCError} If the app installation limit is reached or if validation fails
     */
    ...
    /**
     * Validates whether a user can create or access data based on transaction history retention limits.
     * This method throws a TRPCError if the transaction date is outside the allowed retention period.
     * 
     * @param {string} userId - The user's unique identifier
     * @param {Date} transactionDate - The date of the transaction to validate
     * @throws {TRPCError} If the transaction date is outside the retention period or if validation fails
     */
    public async validateTransactionHistoryAccess(userId: string, transactionDate: Date): Promise<void> {
        try {
            const tier = await getUserTier(userId);
