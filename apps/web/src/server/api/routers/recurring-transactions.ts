...
export const recurringTransactionsRouter = createRouter({
    ...
    getRecurringTransaction: protectedProcedure
        ...
        .query(async ({ ctx, input }) => {
            ...
            if (!recurringTransaction || recurringTransaction.bankAccount.userId !== ctx.userId) {
                throw new TRPCError({
                    code: "NOT_FOUND",
                ...
                switch (input.frequency) {
                    case "WEEKLY":
                        ...
                        while (nextScheduledDate < today) {
                            nextScheduledDate.setDate(nextScheduledDate.getDate() + (7 * input.interval));
                        }
                        break;
                    case "BIWEEKLY":
                        // Find next occurrence by adding 2 weeks
                        while (nextScheduledDate < today) {
                            nextScheduledDate.setDate(nextScheduledDate.getDate() + (14 * input.interval));
                        }
                        break;
                    case "MONTHLY":
                        // Find next occurrence by adding months
                        while (nextScheduledDate < today) {
                            nextScheduledDate.setMonth(nextScheduledDate.getMonth() + (1 * input.interval));
                        }
                        break;
                    case "SEMI_MONTHLY":
                    ...
                    case "ANNUALLY":
                        ...
                        while (nextScheduledDate < today) {
                            nextScheduledDate.setFullYear(nextScheduledDate.getFullYear() + (1 * input.interval));
                        }
                        break;
                    default:
    ...
    updateRecurringTransaction: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            const existingRecurringTransaction = await prisma.recurringTransaction.findUnique({
                where: { id: input.id },
                include: {
                    bankAccount: {
                        select: {
            ...
            if (!existingRecurringTransaction || existingRecurringTransaction.bankAccount.userId !== ctx.userId) {
                throw new TRPCError({
                    code: "NOT_FOUND",
                    message: "Recurring transaction not found",
                });
            ...
            if (input.data.amount !== undefined && existingRecurringTransaction.affectAvailableBalance) {
            ...
            if (input.data.frequency || input.data.startDate || input.data.interval ||
                input.data.dayOfMonth || input.data.dayOfWeek) {

                const frequency = input.data.frequency || existingRecurringTransaction.frequency;
                const interval = input.data.interval || existingRecurringTransaction.interval;
                ...
                if (nextScheduledDate < today) {
                    switch (frequency) {
                        case "WEEKLY":
                            while (nextScheduledDate < today) {
                                nextScheduledDate.setDate(nextScheduledDate.getDate() + (7 * interval));
                            }
                            break;
                        case "BIWEEKLY":
                            while (nextScheduledDate < today) {
                                nextScheduledDate.setDate(nextScheduledDate.getDate() + (14 * interval));
                            }
                            break;
                        case "MONTHLY":
                            while (nextScheduledDate < today) {
                        ...
                        case "ANNUALLY":
                            while (nextScheduledDate < today) {
                                nextScheduledDate.setFullYear(nextScheduledDate.getFullYear() + interval);
                            }
                            break;
                        default:
                            nextScheduledDate = new Date();
    ...
    addTags: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            const updatedTags = [...new Set([...recurringTransaction.tags, ...input.tags])];
    ...
    detectRecurringTransactions: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            const { bankAccountId, minConfidence, minimumOccurrences, lookbackDays } = input;
            ...
            for (const [merchantName, merchantTransactions] of Object.entries(groupedByMerchant)) {
                ...
                for (let i = 1; i < merchantTransactions.length; i++) {
                    const dayDiff = Math.round(
                        (merchantTransactions[i].date.getTime() - merchantTransactions[i - 1].date.getTime()) /
                        (1000 * 60 * 60 * 24)
                    );
                ...
                const confidenceScore = Math.max(0, Math.min(1, 1 - (stdDev / maxAllowedStdDev)));
                ...
                if (frequency === "MONTHLY" || frequency === "SEMI_MONTHLY" || frequency === "ANNUALLY") {
