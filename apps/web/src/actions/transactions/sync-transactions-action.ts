 ...
 * 
 * This server action triggers the Trigger.dev job to sync transactions after
 * a user has connected their account via Plaid.
 *
 * @example
...
export const syncTransactionsAction = authActionClient
    ...
    .action(async ({ ctx: { user }, parsedInput: { accessToken, itemId, institutionId, userId } }) => {
        try {
