...
export const createPlaidLinkTokenAction = authActionClient
  ...
  .action(async ({ ctx: { user }, parsedInput: { accessToken } }) => {
    try {
      ...
      /**
       * Step 1: Create or update customer in Plain CRM This creates a customer
       * profile or updates an existing one based on user email
       */

      /**
       * Creates a Plaid link token for the authenticated user
       *
       * This server action generates a Plaid link token that allows the client
       * to initialize the Plaid Link flow for connecting financial
       ...
       *   });
       *
       * @param {object} params - The parameters for creating the link token
       ...
       * @param {string | null} params.parsedInput.accessToken - Optional Plaid
       *   access token for updating an existing connection
       * @returns {Promise<string>} A Plaid link token string that can be used
       *   to initialize Plaid Link on the client
      ...
      /**
       * Step 5: Error handling Let next-safe-action handle the error formatting
