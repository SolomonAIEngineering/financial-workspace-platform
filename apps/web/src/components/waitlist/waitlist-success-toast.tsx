 ...
 * @property {string} featureName - The name of the feature for which the user
 *   joined the waitlist. This will be displayed in the toast message.
 * @property {number} [autoCloseDelay=5000] - Time in milliseconds after which
 *   the toast will automatically close. Default is 5000ms (5 seconds). Default
 *   is `5000`
...
export function WaitlistSuccessToast({
  ...
  /**
   * Effect to automatically close the toast after the specified delay. The
   * timer is cleared if the toast becomes invisible or when the component
   * unmounts.
   */
  useEffect(() => {
    if (isVisible) {
