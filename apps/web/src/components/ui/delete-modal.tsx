 ...
 * 
 * A comprehensive, customizable confirmation modal for delete operations with double confirmation.
 * 
 * @example
 * ```tsx
 * // Basic usage
...
export function DeleteModal({
    ...
    const isConfirmationValid = !requireDoubleConfirmation || !isSecondStage || confirmationText === confirmationWord;
        ...
        <AlertDialog open={open} onOpenChange={handleDialogOpenChange}>
            ...
            <AlertDialogContent className={cn("", className)}>
                ...
                {isSecondStage && requireDoubleConfirmation && (
                    <div className="my-4">
                        <div className="mb-2 text-sm font-medium">Type <span className="font-bold text-destructive">{confirmationWord}</span> to confirm:</div>
                        <Input
                            value={confirmationText}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setConfirmationText(e.target.value)}
                            className={cn(
                                "",
                                confirmationText && confirmationText !== confirmationWord && "border-destructive"
                            )}
