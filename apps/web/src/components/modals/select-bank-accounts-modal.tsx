...
const AccountTypeBadge = ({ type }: { type: string }) => {
    const typeColorMap: Record<string, { bg: string; text: string; icon?: JSX.Element }> = {
        CHECKING: {
            bg: 'bg-blue-50',
            text: 'text-blue-700',
    ...
    if (sendSupport.status === 'hasSucceeded') {
        return (
            <motion.div
                ...
                className="flex h-[250px] flex-col items-center justify-center space-y-3 bg-zinc-50/50 rounded-xl p-8"
            >
        ...
        <Form {...form}>
            <form onSubmit={handleOnSubmit} className="space-y-4">
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            <FormLabel className="text-sm font-medium text-zinc-800">Message</FormLabel>
                            <FormControl>
                                <Textarea
                                    placeholder="Describe the issue you're facing, along with any relevant information. Please be as detailed and specific as possible."
                                    className="min-h-[150px] resize-none border-zinc-200 rounded-xl focus:ring-1 focus:ring-black focus:border-black transition-all w-full p-[2%]"
                                    autoFocus
                                    {...field}
                                />
                            </FormControl>
                ...
                <div className="flex justify-end">
                    <Button
                        ...
                        className="mt-4 bg-black hover:bg-zinc-800 text-white px-6 py-2.5 rounded-xl transition-colors shadow-lg shadow-black/5 hover:shadow-black/10"
                    >
...
export function SelectBankAccountsModal({
    ...
    const connectBankAction = useAction(handleBankConnectionAction, {
        ...
        onSuccess: ({ data }) => {
            console.log('✅ Bank Connection Success:', {
                ...
                selectedAccounts: form.getValues('accounts')?.filter(acc => acc.enabled)
            });
            ...
            if (data?.id) {
                console.log('📝 Setting runId with all response data:', data.id, {
                    ...data
                });
                if (data?.id) {
    ...
    useEffect(() => {
        const subscription = form.watch((value) => {
            if (value.accounts && Array.isArray(value.accounts)) {
                const count = value.accounts.filter(account => account?.enabled).length;
        ...
        try {
            ...
            try {
                if (values.accounts && Array.isArray(values.accounts)) {
                    enabledAccounts = values.accounts.filter((account: any) => account?.enabled);
                } else {
                    console.error('accounts is not an array or is undefined', values.accounts);
                }
            } catch (err) {
    ...
    }, [isOpen, provider, form, ref, token, institution_id, itemId, userId, teamId]);

    // Toggle all accounts selection
    const toggleAllAccounts = (enabled: boolean) => {
        const currentAccounts = form.getValues('accounts');
        form.setValue('accounts', currentAccounts.map(account => ({ ...account, enabled })));
    };
    ...
    return (
        ...
        >
            <DialogContent
                ...
                className="max-w-md rounded-2xl border-0 shadow-[0_25px_80px_-12px_rgba(0,0,0,0.25)] p-0 overflow-hidden transition-all bg-white"
            >
                    ...
                    <Tabs defaultValue="select-accounts" value={activeTab}>
                        <TabsContent value="select-accounts" className="mt-0">
                            <>
                                <DialogHeader className="mb-6">
                                    <DialogTitle className="text-xl font-semibold text-black tracking-tight">Select Accounts</DialogTitle>
                                    <DialogDescription className="text-sm text-zinc-600 mt-2">
                                        Choose the accounts you want to connect. You can enable
                                        or disable them later in settings.
                                    </DialogDescription>
                                </DialogHeader>
                                ...
                                <div className="relative mb-4">
                                    ...
                                    <Input
                                        ...
                                        className="w-full pl-10 py-2.5 rounded-xl border-zinc-200 focus:ring-1 focus:ring-black focus:border-black text-sm"
                                    />
                                </div>
                                ...
                                <div className="flex justify-between items-center mb-4 pb-3 border-b border-zinc-100">
                                    <div className="text-sm text-zinc-700">
                                        <span className="font-semibold">{selectedCount}</span> of <span className="font-semibold">{accounts.length}</span> selected
                                    </div>
                                    <div className="flex gap-2">
                                        <Button
                                            ...
                                            className="text-xs py-1 h-auto border-zinc-200 hover:bg-zinc-50 rounded-lg"
                                        >
                                            Select all
                                        </Button>
                                        <Button
                                ...
                                <Form {...form}>
                                    ...
                                    <div className="relative scrollbar-hide h-[350px] overflow-auto pb-[100px] px-1">
                                        {loading && <RowsSkeleton />}

                                        {accounts.length === 0 && !loading && (
                                            <div className="flex flex-col items-center justify-center h-32 text-center">
                                                <p className="text-zinc-500 mb-2">No accounts found</p>
                                                <Button
                                                    variant="outline"
                                                    onClick={() => setActiveTab('support')}
                                                    className="mt-2 text-xs border-zinc-200 rounded-lg"
                                                >
                                                    Contact Support
                                                </Button>
                                            </div>
                                        ...
                                        {filteredAccounts.length === 0 && searchQuery && !loading && (
                                            <div className="flex flex-col items-center justify-center h-32 text-center">
                                                <p className="text-zinc-500 mb-2">No matching accounts found</p>
                                                <Button
                                                    variant="outline"
                                                ...
                                                <FormField
                                                    ...
                                                    render={({ field }) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <FormItem
                                                                    className={`flex flex-row items-center p-4 hover:bg-zinc-50 rounded-xl transition-all duration-200 
                                                                    border ${isEnabled ? 'border-black/10 bg-black/[0.02]' : 'border-zinc-100'} 
                                                                    hover:border-zinc-200 ${isEnabled ? 'shadow-sm' : ''}`}
                                                                >
                                                                    <FormLabel className="mr-4 flex w-full items-center space-x-4 cursor-pointer">
                                                                        <Avatar className="size-[42px] border border-zinc-100 shadow-sm">
                                                                            ...
                                                                            <AvatarFallback className="text-[12px] bg-gradient-to-b from-zinc-50 to-zinc-100 text-zinc-800">
                                                                        ...
                                                                        <div className="flex w-full items-center justify-between">
                                                                            ...
                                                                            <span className={`text-sm font-medium ${isEnabled ? 'text-black' : 'text-zinc-700'}`}>
                                                                                <FormatAmount
                                        ...
                                        <div className="fixed right-0 bottom-0 left-0 z-10 bg-white bg-opacity-98 backdrop-blur-lg px-8 pt-6 pb-8 border-t border-zinc-100 shadow-[0_-10px_30px_-5px_rgba(0,0,0,0.08)]">
                                            <div className="flex items-center justify-between mb-3">
                                                <div className="text-sm text-zinc-600">
                                                    {selectedCount > 0 ? (
                                                        <span>Connecting <span className="font-medium">{selectedCount}</span> {selectedCount === 1 ? 'account' : 'accounts'}</span>
                                                    ) : (
                                                        <span className="text-amber-600">Select at least one account</span>
                                                    )}
                                                </div>
                                            </div>
                                            <Button
                                                className={`w-full py-3 rounded-xl font-medium transition-all text-sm shadow-xl ${selectedCount > 0
                                                    ? 'bg-black hover:bg-zinc-800 text-white shadow-black/5 hover:shadow-black/10 transform hover:-translate-y-0.5'
                                            ...
                                            >
                                                {isSubmitting || connectBankAction.status === 'executing' ? (
                                                    <motion.span
                                            ...
                                            <div className="mt-4 flex justify-center items-center space-x-3">
                                                <Button
                                                    type="button"
                                                    variant="ghost"
                                                    className="text-sm text-zinc-600 hover:text-zinc-900 transition-colors hover:bg-transparent"
                                                    onClick={() => setActiveTab('support')}
                                                >
                                                    Need help?
                                                </Button>
                        ...
                        <TabsContent value="support">
                            <div className="mb-8 flex items-center space-x-3">
                                <button
                                    type="button"
                                    className="flex items-center justify-center p-2 rounded-full hover:bg-zinc-100 transition-colors"
