 ...
 * transactions. This file is the central repository for all tag colors, default
 * values, sample transactions, and filter field definitions used throughout the
...
/**
 * Tag color definitions for different tag categories. Provides consistent
 * styling for badges and indicators across the UI.
 *
...
/**
 * Sample transaction data for testing and development. Each record represents a
 * financial transaction with properties matching the ColumnSchema. The data
 * includes various types of transactions like income, expenses, transfers, and
...
/**
 * Filter field definitions for the transactions data table. Each filter object
 * defines a UI control that users can interact with to filter transaction
 * data.
 *
 * The filters include:
 *
...
export const filterFields = [
  ...
  },
  {
    ...
    options: Array.from(new Set(data.flatMap((item) => item.tags))).map(
  ...
  },
  {
    ...
    type: 'checkbox',
    options: Array.from(new Set(data.map((item) => item.bankAccountName))).map(
      (name) => ({
        label: name || 'Unknown',
        value: name,
      })
  ...
  },
  {
    ...
    options: Array.from(
      new Set(
        data
          .filter((item) => item.budgetCategory)
          .map((item) => item.budgetCategory as string)
      )
    ).map((category) => ({
      label: category,
      value: category,
