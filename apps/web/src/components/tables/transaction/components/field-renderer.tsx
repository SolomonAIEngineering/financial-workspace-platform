...
/**
 * FieldRenderer component - Renders editable or non-editable field based on
 * conditions
 *
 * This component centralizes the logic for rendering transaction fields,
 * handling both edit mode and view mode displays based on the current context
  ...
  if (hasValue || field === 'name' || field === 'category' || isBoolean) {
    return (
      <DetailRow
        ...
        label={label}
        value={
          isBoolean ? (value ? 'Yes' : 'No') : (formattedValue as string) || '-'
        }
        tooltip={tooltip}
        monospace={monospace}
        isAmount={isAmount}
