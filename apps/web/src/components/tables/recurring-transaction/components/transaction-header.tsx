import { AlertCircle, Calendar, CheckCircle2, ChevronDown, CircleDollarSign, Clock, CreditCard, PauseCircle, TrendingDown, TrendingUp, User, Zap } from 'lucide-react';
...
/**
 * A modern, visually appealing header component for displaying key recurring transaction information
 */
export function TransactionHeader({
    ...
    const daysLeft = nextExecutionDate
        ? Math.max(0, Math.ceil(
            (new Date(nextExecutionDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)
        ))
        : null;
    ...
    const statusBg = statusBgColors[status.toLowerCase()] || 'from-blue-50 to-white';
    ...
    const importanceStyle = importanceStyles[importance || 'MEDIUM'] || importanceStyles.MEDIUM;
    ...
    const renderMerchantIcon = () => {
        ...
        return (
            <div className={cn(
                "flex h-14 w-14 rounded-full items-center justify-center text-xl font-semibold",
                backgroundClass
            )}>
                {firstLetter}
            </div>
                ...
                <div className="p-5">
                    ...
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div className="flex flex-col items-end">
                            ...
                            <span className={cn(
                                "rounded-full border px-3 py-1 text-xs font-medium mt-2",
                                importanceStyle
                            )}>
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-between border-t border-blue-100 pt-4 pb-1 text-sm font-medium text-blue-600 focus:outline-none"
                    >
