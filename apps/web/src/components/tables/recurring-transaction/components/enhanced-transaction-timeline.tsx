...
import { addDays, addMonths, addWeeks, differenceInDays, format, isBefore, isSameDay, parseISO, subMonths } from 'date-fns';
...
/**
 * A visually impressive timeline component for displaying recurring transaction history and forecasts
 */
export function EnhancedTransactionTimeline({
    ...
    const [visibleHistoryItems, setVisibleHistoryItems] = useState(INITIAL_HISTORY_ITEMS);
    const [visibleFutureItems, setVisibleFutureItems] = useState(INITIAL_FUTURE_ITEMS);
    ...
    const handleHistoryScroll = useCallback(() => {
        ...
        if (newCanScrollUp !== historyScrollState.canScrollUp ||
        ...
        if (scrollTop > scrollHeight - clientHeight - 100 &&
            ...
            setVisibleHistoryItems(prev => Math.min(prev + 5, displayedHistoryCount * 3));
        }
    ...
    const handleFutureScroll = useCallback(() => {
        ...
        if (newCanScrollUp !== futureScrollState.canScrollUp ||
    ...
    const handleFutureKeyDown = useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
        if (!futureScrollRef.current) return;
    ...
    const generateFutureDates = useCallback((startDate: Date | string, count: number) => {
    ...
    const generateHistoryDates = useCallback((endDate: Date | string, count: number) => {
    ...
    const totalSum = useMemo(() => historyDates.length * transaction.amount,
    ...
    const frequencyLabel = useMemo(() => {
        switch (transaction.frequency) {
            ...
            default: return transaction.frequency;
        }
    ...
    const monthlyPattern = useMemo(() => {
        ...
        const seed = transaction.id ? parseInt(transaction.id.substring(0, 8), 16) : 123456;
    ...
    const historyItems = useMemo(() => {
        return historyDates.slice(0, visibleHistoryItems).map((date, index) => (
            <HistoryTimelineItem
                key={`history-${index}`}
                date={date}
                index={index}
    ...
    const futureItems = useMemo(() => {
        return futureDates.slice(0, visibleFutureItems).map((date, index) => (
            <FutureTimelineItem
                key={`future-${index}`}
                date={date}
                index={index}
    ...
    return (
        <div className="space-y-6">
            ...
            <motion.div
                ...
                className="rounded-xl border border-blue-100 bg-gradient-to-br from-blue-50 to-indigo-50 p-5 shadow-sm"
            >
                <div className="mb-4 flex items-center gap-2">
                    <History className="h-5 w-5 text-blue-600" />
                    <h3 className="text-sm font-medium text-blue-800">Transaction Timeline</h3>
                </div>
                ...
                <div className="mb-4 grid grid-cols-2 gap-x-6 gap-y-3">
                    <div>
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <p className="text-sm font-medium text-gray-800">{frequencyLabel}</p>
                        </div>
                    </div>
                    <div>
                ...
                <div className="mb-2">
                    ...
                    <div className="mt-3 h-16 w-full">
                        <div className="flex h-full items-end gap-1">
                            ...
                            {Array.from({ length: Math.min(6, futureDates.length) }).map((_, idx) => {
                    ...
                    <div className="mt-1 flex justify-between text-xs text-gray-500">
                        <span>{historyDates.length > 0 ? format(historyDates[0], 'MMM d') : 'Past'}</span>
                        <span>Today</span>
                        <span>{futureDates.length > 0 ? format(futureDates[5], 'MMM d') : 'Future'}</span>
                    </div>
                </div>
                            ...
                            <div className="mt-4 h-32 w-full">
                                <div className="flex h-24 items-end justify-between">
                                    {monthlyPattern.map((month, idx) => (
                                        <div key={`month-${idx}`} className="flex w-[7.5%] flex-col items-center">
                                            <div
                                            ...
                                            />
                                            <span className="mt-1 text-[10px] text-gray-500">{month.name}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
            ...
            <div className="relative space-y-6 py-4">
                ...
                <div className="relative">
                    <h4 className="mb-4 pl-16 font-medium text-gray-800 flex items-center gap-2">
                    ...
                    {historyDates.length > 0 ? (
                        <div className="relative">
                            ...
                            <div className={cn(
                                "absolute left-16 right-0 top-0 h-6 bg-gradient-to-b from-white to-transparent z-10 pointer-events-none transition-opacity duration-300",
                            ...
                            <div className={cn(
                                "absolute left-16 right-0 bottom-0 h-6 bg-gradient-to-t from-white to-transparent z-10 pointer-events-none transition-opacity duration-300",
                            ...
                            {historyScrollState.canScrollDown && (
                                <button
                                    ...
                                    className="absolute -bottom-2 right-3 z-20 flex h-6 w-6 items-center justify-center rounded-full bg-blue-100 text-blue-800 shadow-sm hover:bg-blue-200"
                        ...
                        <p className="ml-16 text-sm italic text-gray-500">No historical transactions available</p>
                    )}
                </div>
                ...
                <div className="relative mb-8 mt-8">
                    {/* Connector Lines */}
                    <div className="absolute left-8 top-1/2 h-0.5 w-8 bg-indigo-300"></div>

                    <div
                        className="absolute left-8 top-1/2 flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-white shadow-md z-10"
                    >
                        <div className="h-6 w-6 rounded-full bg-indigo-500 animate-pulse"></div>
                    </div>
                    <div
                        className="ml-16 rounded-lg border border-indigo-100 bg-gradient-to-r from-indigo-50 to-white p-3 shadow-sm transition-all duration-200 hover:shadow-md"
                    >
                        <div className="flex items-center gap-2">
                            <Clock className="h-4 w-4 text-indigo-600" />
                            <p className="text-sm font-medium text-indigo-900">Today - {format(new Date(), 'MMMM d, yyyy')}</p>
                        </div>
                        <div className="mt-1 flex items-center gap-1.5">
                            <div className="h-1.5 w-1.5 rounded-full bg-indigo-500 animate-pulse"></div>
                            <p className="text-xs text-indigo-700">
                                Next payment in {differenceInDays(nextExecutionDate, new Date())} days
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="relative">
                    ...
                    {futureDates.length > 0 ? (
                        <div className="relative">
                            ...
                            {futureScrollState.canScrollDown && (
                                <button
                                    ...
                                    className="absolute -bottom-2 right-3 z-20 flex h-6 w-6 items-center justify-center rounded-full bg-indigo-100 text-indigo-800 shadow-sm hover:bg-indigo-200"
                                    aria-label="Scroll down"
                                >
                                    <ChevronDown className="h-4 w-4" />
                                </button>
                        ...
                        <p className="ml-16 text-sm italic text-gray-500">No future transactions scheduled</p>
                    )}
                </div>
            </div>
        </div>
...
const HistoryTimelineItem = React.memo(({ date, index, transaction }: {
    date: Date;
    index: number;
    transaction: RecurringTransactionSchema;
}) => (
    ...
    >
        {/* Connector Line */}
        <div className="absolute -left-4 top-4 h-0.5 w-4 bg-blue-300"></div>

        {/* Date indicator */}
        <div className="absolute -left-12 top-0 flex h-8 w-8 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-blue-400 to-blue-500 shadow-md z-10">
            <CheckCircle className="h-4 w-4 text-white" />
        </div>
        {/* Content */}
        <div
            className="rounded-lg border border-gray-100 bg-white p-3 shadow-sm hover:shadow-md transition-all duration-200"
        >
            <div className="mb-1 flex items-center justify-between">
                <p className="text-sm font-medium text-gray-900">{format(date, 'MMMM d, yyyy')}</p>
...
// Extracted into separate components to reduce re-renders
const FutureTimelineItem = React.memo(({ date, index, transaction }: {
    date: Date;
    index: number;
    transaction: RecurringTransactionSchema;
}) => (
    ...
    >
        {/* Connector Line */}
        <div className="absolute -left-4 top-4 h-0.5 w-4 bg-indigo-300"></div>

        {/* Date indicator */}
        <div className="absolute -left-12 top-0 flex h-8 w-8 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 shadow-md z-10">
