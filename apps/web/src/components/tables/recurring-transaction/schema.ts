...
/**
 * Defines the data schema for recurring transactions using Zod. This file
 * contains type definitions, validation schemas, and enum values used
 * throughout the recurring transaction components.
 *
 * @file Schema.ts
 */

/**
 * Valid status values for recurring transactions. These statuses represent the
 * current state of a recurring transaction.
 *
 * @property active - Transaction is currently active and will execute as
 *   scheduled
...
/**
 * Frequency options for recurring transactions. Defines how often a recurring
...
/**
 * Types of recurring transactions. Categorizes the transaction's purpose.
 *
...
/**
 * Importance levels for recurring transactions. Indicates the priority or
...
/**
 * Zod schema for recurring transactions. Defines the structure, types, and
 * validation rules for recurring transaction data.
 *
...
/**
 * TypeScript type for recurring transactions, derived from the Zod schema. Use
 * this type when working with recurring transaction objects throughout the
 * application.
 *
 * @example
 *   ```tsx
...
/**
 * Zod schema for filtering recurring transactions. Defines the structure of
 * filter parameters used in the data table and API requests. This schema allows
 * for filtering by multiple criteria simultaneously.
 *
 * @remarks
 *   Date ranges and amount ranges are represented as arrays with exactly two
 *   items (start and end values) to support range-based filtering.
 */
 ...
 * this type when implementing filter functionality for recurring transactions.
 *
 * @example
 *   ```tsx
 ...
 *   ```;
 */
export type RecurringTransactionFilterSchema = z.infer<
  typeof recurringTransactionFilterSchema
>;

/**
 * Base schema for chart data related to recurring transactions. Used for
 * visualizing transaction data in time-series charts.
 *
