 ...
 */
export function formatFrequency(frequency: string, dayOfMonth?: number, dayOfWeek?: number): string {
    switch (frequency) {
        case 'DAILY':
            return 'Daily';
        case 'WEEKLY':
 ...
 */
export function getStatusBadgeType(status: string): 'active' | 'pending' | 'warning' | 'error' | 'cancelled' | 'paused' | 'completed' | 'unknown' {
    const statusMap: Record<string, 'active' | 'pending' | 'warning' | 'error' | 'cancelled' | 'paused' | 'completed' | 'unknown'> = {
        ACTIVE: 'active',
        PENDING: 'pending',
        FAILED: 'error',
        CANCELLED: 'cancelled',
 ...
 */
export function getImportanceBadgeType(importance: string | undefined): 'default' | 'success' | 'warning' | 'info' | 'error' {
    const importanceMap: Record<string, 'default' | 'success' | 'warning' | 'info' | 'error'> = {
        LOW: 'info',
        MEDIUM: 'default',
        HIGH: 'warning',
        CRITICAL: 'error',
 ...
 */
export function getExecutionStatusBadgeType(status: string): 'success' | 'warning' | 'info' | 'error' | 'default' {
