...
/**
 * A beautiful, modern component that displays detailed information about a recurring transaction.
 * Organizes information into collapsible sections for better readability and user experience.
 */
export function RecurringTransactionSheetDetails() {
  ...
  const transaction = (selectedRow?.original as RecurringTransactionSchema) || ({} as RecurringTransactionSchema);
      ...
      {!transaction ? (
        <div className="flex h-64 w-full items-center justify-center">
          <div className="flex flex-col items-center text-center">
            ...
            <h3 className="mt-2 text-lg font-medium text-gray-600">No transaction selected</h3>
            <p className="mt-1 text-sm text-gray-500">Please select a transaction to view details</p>
          </div>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          >
            <DetailRow
              ...
              isBadge={true}
              badgeType={transaction.status === 'active' ? 'success' :
                transaction.status === 'paused' ? 'warning' :
          ...
          >
            <DetailRow
              label="Amount"
              value={formatAmount(transaction.amount, transaction.currency || 'USD')}
              isAmount={true}
          ...
          >
            <DetailRow
              ...
              value={formatFrequency(
                ...
                transaction.dayOfMonth !== null ? transaction.dayOfMonth : undefined,
                transaction.dayOfWeek !== null ? transaction.dayOfWeek : undefined
              )}
            />
            <DetailRow
              label="Interval"
              value={transaction.interval ? transaction.interval.toString() : '1'}
          ...
          >
            ...
            />
            {transaction.minBalanceRequired !== null && transaction.minBalanceRequired !== undefined && (
              <DetailRow
                label="Minimum Balance"
                value={formatAmount(transaction.minBalanceRequired, transaction.currency || 'USD')}
                isAmount={true}
                amountType="neutral"
          ...
          >
            ...
            />
            {transaction.isVariable && transaction.allowedVariance !== undefined && (
              <DetailRow
          ...
          >
            ...
            <DetailRow
              ...
              isBadge={true}
              badgeType={transaction.lastExecutionStatus?.toLowerCase()?.includes('success') ? 'success' :
                transaction.lastExecutionStatus?.toLowerCase()?.includes('fail') ? 'error' : 'info'}
            />
            <DetailRow
