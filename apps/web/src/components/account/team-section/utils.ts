...
export const getCurrencyInfo = (code: string): Currency => {
    ...
    return currency || { code: code || 'USD', name: 'Unknown Currency', symbol: '$' };
};
 ...
 */
export const validateTeamSlug = (slug: string): { isValid: boolean; errorMessage: string } => {
    if (!slug.trim()) {
 ...
 */
export const countTeamOwners = (team: { usersOnTeam?: { role: TeamRole }[] }): number => {
