...
export const FloatingComments = () => {
  ...
  const suggestionEntriesMap = useMemo(() => {
    const suggestionEntriesMap: Record<
      string,
    ...
    suggestionIds.forEach((id) => {
      ...
      suggestionEntriesMap[id] = entries;
    });

    return suggestionEntriesMap;
  ...
  const renderFloatingDiscussion = React.useCallback(() => {
    ...
    Object.entries(suggestionEntriesMap).forEach(([id, entries]) => {
      if (!id) return;
    ...
    forceUpdate();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data?.discussions, editorContainerRef, isOverlapWithEditor]);
  ...
  return (
    <>
      ...
      {Object.entries(suggestionEntriesMap).map(
        ([id, entries]) =>
          id && (
...
const FloatingCommentsContent = ({
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'absolute right-20 w-72 animate-fade-in cursor-pointer rounded-lg border bg-white p-3 transition-transform duration-200',
        '[&[data-hover=true][data-active=false]]:-translate-x-2 [&[data-hover=true][data-active=false]]:bg-muted/50',
