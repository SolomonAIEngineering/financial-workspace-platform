...
const ConnectTransactionsContext = createContext<ConnectTransactionsContextType | undefined>(undefined);

export function useConnectTransactions() {
    const context = useContext(ConnectTransactionsContext);

    if (!context) {
        throw new Error('useConnectTransactions must be used within a ConnectTransactionsProvider');
    }
