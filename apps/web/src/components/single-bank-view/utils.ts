...
/**
 * CSS to hide scrollbars across different browsers Applied globally to maintain
 * a clean UI without visible scrollbars
 *
 * @constant
 * @type {string}
 */
...
/**
 * Processes transaction data to prepare it for chart visualization Filters
 * transactions by date range and aggregates expenses by day
 *
 * @function
...
/**
 * Calculates monthly financial statistics based on transaction data Includes
 * spending, income, net change and percentage change
 *
 * @function
...
export function calculateMonthlyStats(
  ...
  return {
    spending: spending.toLocaleString('en-US', {
      style: 'currency',
      currency: 'USD',
    }),
    ...
    netChange: (income - spending).toLocaleString('en-US', {
      style: 'currency',
      currency: 'USD',
    }),
    percentChange:
      spending > 0 ? Math.round(((income - spending) / spending) * 100) : 0,
  };
}
