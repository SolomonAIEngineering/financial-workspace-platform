 ...
 */
export function useTransaction(id: string, options: QueryOptions = DEFAULT_QUERY_OPTIONS) {
    ...
    return api.transactions.updateTransaction.useMutation({
        ...
        onError: (_, __, context: any) => {
            if (context?.previousTransactions) {
                trpc.transactions.getTransactions.setData({}, context.previousTransactions);
            }
        ...
        onMutate: async (input) => {
            ...
            const previousTransaction = trpc.transactions.getTransaction.getData({ id: input.id });
...
export function useUpdateTransactionDebounced() {
    ...
    return useCallback(
        (input: { id: string; data: any }) => {
            ...
            void trpc.transactions.getTransaction.setData({ id: input.id }, (prevData) =>
        ...
        onMutate: async (input) => {
            ...
            trpc.transactions.getTransactions.setData({}, (old) =>
                produce(old, (draft) => {
                    ...
                    draft.transactions = draft.transactions.filter(transaction => transaction.id !== input.id);
                })
            );
...
export function useUpdateTransactionNotes() {
    ...
    const updateNotesDebounced = useDebouncedCallback(
        mutation.mutate,
        500
    );
