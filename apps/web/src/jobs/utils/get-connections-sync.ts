 ...
 * institutions.
 *
 * This function identifies bank connections that require data synchronization
 * based on specific criteria, making it ideal for scheduled jobs and automated
 * sync processes. It implements an intelligent prioritization strategy that
 * focuses on connections that:
 *
 ...
 * Results are ordered by sync age (oldest first) and limited to batches of 50
 * to manage system load and API rate limits.
 *
 * @example
 *   ```typescript
 ...
 *   ```
 *
 * @returns A Promise resolving to an array of BankConnection objects that need
 *   synchronization
 */
