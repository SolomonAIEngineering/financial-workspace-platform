...
/**
 * Defines a task to delete a bank connection from a financial provider.
 * This task handles the process of removing a connection from the system and revoking
 * access with the provider.
 * 
 * @remarks
 * The task implements error handling and retry mechanisms to ensure reliable execution.
 * It validates inputs using Zod schema validation before attempting deletion.
 * 
 * @example
 * ```ts
...
export const deleteConnection = schemaTask({
  ...
  schema: z.object({
    ...
    provider: z.string().min(1, "Provider is required")
      .refine(val => ['teller', 'plaid', 'gocardless', 'stripe'].includes(val), {
  ...
  run: async (payload, ctx) => {
    ...
    try {
      ...
      throw new Error(`Failed to delete ${provider} connection: ${errorMessage}`);
    }
  },
  /**
