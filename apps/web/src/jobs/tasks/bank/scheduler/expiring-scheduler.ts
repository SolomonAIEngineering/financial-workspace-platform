...
/**
 * Scheduled task that proactively identifies bank connections approaching expiration
 * due to inactivity and notifies users to maintain uninterrupted access.
 *
 * @remarks
 ...
 * 3. Automatically marks connections as requiring attention if they've been inactive
 *    for 30+ days (likely already expired)
 * 
 * This task helps maintain uninterrupted access to financial data by proactively
 * alerting users before their connections expire, reducing service disruptions.
 * It implements error handling, retry mechanisms, and comprehensive tracing.
 *
 * @example
...
export const expiringSchedulerJob = schedules.task({
  ...
  run: async (payload, ctx) => {
    try {
      ...
      return await logger.trace("expiring-connections-scheduler", async (span) => {
        ...
        await logger.trace("process-expiring-connections", async (subSpan) => {
          subSpan.setAttribute("connectionsToProcess", expiringConnections.length);
          ...
          for (const connection of expiringConnections) {
            ...
            try {
              ...
              subSpan.setAttribute(`connection_${connection.id}_daysInactive`, daysInactive);
              subSpan.setAttribute(`connection_${connection.id}_daysUntilExpiry`, daysUntilExpiry);
        ...
        await logger.trace("mark-expired-connections", async (markSpan) => {
          markSpan.setAttribute("connectionsToMark", expiredConnections.length);
          ...
          for (const connection of expiredConnections) {
            try {
              ...
              const errorMessage = markError instanceof Error
                ? markError.message
                : 'Unknown error';
      ...
      throw new Error(`Failed to process expiring connections: ${errorMessage}`);
    }
  },
  /**
