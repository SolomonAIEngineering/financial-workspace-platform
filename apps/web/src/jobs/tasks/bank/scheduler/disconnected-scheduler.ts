...
/**
 * Scheduled task that identifies and processes bank connections that are in a disconnected
 * or error state. It runs on a daily schedule to maintain data quality and user experience.
 *
 * @remarks
  ...
  run: async (payload, ctx) => {
    try {
      ...
      return await logger.trace("disconnected-connections-scheduler", async (span) => {
        ...
        span.setAttribute("disconnectedConnectionsCount", disconnectedConnections.length);
        logger.info(
        ...
        span.setAttribute("abandonedConnectionsCount", abandonedConnections.length);
        let disabledCount = 0;
        ...
        await logger.trace("disable-abandoned-connections", async (disableSpan) => {
          ...
          for (const connection of abandonedConnections) {
            try {
              ...
              const errorMessage = disableError instanceof Error
