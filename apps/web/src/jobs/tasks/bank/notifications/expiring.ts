 ...
 * @description This job sends detailed notifications to users about bank connections that are
 * approaching their expiration date. It generates both email notifications and in-app 
 * notifications to ensure users are aware of connections that need attention.
 * 
...
export const expiringNotificationsJob = client.defineJob({
  ...
  version: '1.0.0',
  /**
   * Main job execution function that handles sending notifications for expiring connections
   * 
   ...
   * @param payload.daysUntilExpiry - Number of days until the connection expires
   * @param payload.daysInactive - Number of days the connection has been inactive
   * @param payload.accountCount - Number of accounts linked through this connection
 ...
 * @param params.daysInactive - Number of days since the connection was last active
 ...
 * @param params.institutionName - Name of the financial institution
 * @param params.name - User's name for personalized greeting
 * @param params.reconnectUrl - URL for the user to click to reconnect their account
