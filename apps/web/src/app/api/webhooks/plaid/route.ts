 ...
 *
 * This module implements a secure API route to handle incoming webhooks from
 * Plaid. It processes transaction updates, validates webhook payloads, and
 * triggers synchronization jobs for linked bank accounts.
 *
 * @see https://plaid.com/docs/api/webhooks/ - Plaid Webhook Documentation
 */

/**
 * List of allowed Plaid IPs for webhook calls This is a security measure to
 * ensure that only legitimate Plaid servers can send webhooks.
 *
 ...
 *
 * This function processes incoming webhook events from Plaid and triggers
 * appropriate actions:
 *
 ...
 *
 *    - For SYNC_UPDATES_AVAILABLE, DEFAULT_UPDATE, INITIAL_UPDATE: Triggers a
 *         standard sync job
 *    - For HISTORICAL_UPDATE: Determines if manual sync is needed (based on
 *         connection age) and triggers appropriate sync
 *
