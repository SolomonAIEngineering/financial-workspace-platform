...
test('deletes the keys', async (t) => {
  ...
  const softDeleteRes = await h.post<
    V1ApisDeleteKeysRequest,
    V1ApisDeleteKeysResponse
  >({
  ...
  expect(
    softDeleteRes.status,
    `expected 200, received: ${JSON.stringify(softDeleteRes)}`,
  ).toBe(200)
  ...
  const hardDeleteRes = await h.post<
    V1ApisDeleteKeysRequest,
    V1ApisDeleteKeysResponse
  >({
    url: '/v1/apis.deleteKeys',
  ...
  expect(
    hardDeleteRes.status,
    `expected 200, received: ${JSON.stringify(hardDeleteRes)}`,
  ).toBe(200)
