...
export const registerV1KeysGetVerifications = (app: App) =>
  app.openapi(
    ...
    async (c): Promise<RouteConfigToTypedResponse<typeof route>> => {
      ...
      if (keyId) {
        const data = await cache.keyById.swr(keyId, async (keyId) => {
          ...
          return {
            ...
            permissions: dbRes.permissions.map(
              (p: { permission: { name: string } }) => p.permission.name,
            ),
            roles: dbRes.roles.map(
              (p: { role: { name: string } }) => p.role.name,
            ),
              ...
              ...apiIds.map(
                (apiId) =>
                  `api.${apiId}.read_key` satisfies z.infer<
                    typeof solomonAIPermissionValidation
                  >,
              ),
            ),
          ),
      ...
      if (
        ids.some(({ workspaceId }) => workspaceId !== authorizedWorkspaceId)
      ) {
        throw new UnkeyApiError({
          code: 'UNAUTHORIZED',
      ...
      if (
        ...
        return c.json({ verifications: [] }, 200)
      }
      return c.json(
        {
