...
describe('correct roles', () => {
  describe.each([
    { name: 'list override', roles: ['ratelimit.*.read_override'] },
  ])('$name', ({ roles }) => {
...
describe('incorrect roles', () => {
  describe.each([
    ...
    {
      name: 'wrong namespace permissions',
      roles: ['ratelimit.othernamespace.read_override'],
    },
    {
      name: 'expired token',
      roles: ['ratelimit.*.read_override'],
      tokenExpired: true,
    },
    {
      name: 'invalid token',
