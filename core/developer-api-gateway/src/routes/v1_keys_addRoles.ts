...
export const registerV1KeysAddRoles = (app: App) =>
  app.openapi(route, async (c): Promise<RouteConfigToTypedResponse<typeof route>> => {
    ...
    const requestedNames = req.roles
      .filter((r) => 'name' in r)
      .map((r) => r.name!)
    ...
    for (const role of req.roles) {
      ...
      if (
        'name' in role &&
        !existingRoles.some((ep) => ep.name === role.name)
      ) {
        if (!role.create) {
          throw new UnkeyApiError({
            code: 'NOT_FOUND',
    ...
    const addRoles = allRoles.filter(
      (ar) => !connectedRoles.some((cp) => cp.roleId === ar.id),
    )
    ...
    return c.json(allRoles.map((p) => ({ id: p.id, name: p.name })), 200)
  })
