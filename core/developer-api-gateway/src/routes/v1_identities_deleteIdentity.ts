...
import { eq, inArray, schema } from '@solomonai/db'
...
export const registerV1IdentitiesDeleteIdentity = (app: App) =>
  app.openapi(
    ...
    async (c): Promise<RouteConfigToTypedResponse<typeof route>> => {
      ...
      const auth = await rootKeyAuth(
        ...
        buildUnkeyQuery(({ or }) =>
          or(
            ...
            `identity.${identityId}.delete_identity`,
          ),
        ),
      )
      ...
      const deleteRatelimitIds = identity.ratelimits
      ...
      await db.primary.transaction(async (tx) => {
        if (deleteRatelimitIds.length > 0) {
          await tx
            ...
            .where(inArray(schema.ratelimits.id, deleteRatelimitIds))
        }
