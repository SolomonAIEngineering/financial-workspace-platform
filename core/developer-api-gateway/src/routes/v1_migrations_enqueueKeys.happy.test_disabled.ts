...
test('creates key', async (t) => {
  ...
  expect(
    res.status,
    `expected 202, received: ${JSON.stringify(res, null, 2)}`,
  ).toBe(202)
  ...
  await vi.waitFor(
    async () => {
      const found = await h.db.primary.query.keys.findMany({
        where: (table, { eq }) =>
          eq(table.keyAuthId, h.resources.userKeyAuth.id),
      })
...
test('migrate and verify a key', async (t) => {
  ...
  await vi.waitFor(
    async () => {
      ...
      expect(
        verifyRes.status,
        `expected 200, received: ${JSON.stringify(verifyRes)}`,
      ).toBe(200)
