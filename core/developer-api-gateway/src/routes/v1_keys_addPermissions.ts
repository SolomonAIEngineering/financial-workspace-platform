...
export const registerV1KeysAddPermissions = (app: App) =>
  app.openapi(
    ...
    async (c): Promise<RouteConfigToTypedResponse<typeof route>> => {
      ...
      const requestedIds = req.permissions
        .filter((p) => 'id' in p)
        .map((p) => p.id!)
      const requestedNames = req.permissions
        .filter((p) => 'name' in p)
        .map((p) => p.name!)

      const [key, existingPermissions, connectedPermissions] =
        await Promise.all([
              ...
              and(
                ...
                or(
                  ...
                  requestedNames.length > 0
                    ? inArray(table.name, requestedNames)
                    : undefined,
                ),
              ),
          }),
      ...
      for (const permission of req.permissions) {
        if (
          'id' in permission &&
          !existingPermissions.some((ep) => ep.id === permission.id)
        ) {
          throw new UnkeyApiError({
            code: 'NOT_FOUND',
        ...
        if (
          'name' in permission &&
          !existingPermissions.some((ep) => ep.name === permission.name)
        ) {
      ...
      return c.json(
        allPermissions.map((p) => ({ id: p.id, name: p.name })),
        200,
      )
    },
  )
