...
`,
                  ...
                  hash: z
                    .object({
                      value: z
                        .string()
                        .openapi({ description: 'The hashed and encoded key' }),
...
export const registerV1MigrationsEnqueueKeys = (app: App) =>
  app.openapi(route, async (c) => {
      ...
      buildUnkeyQuery(({ or, and }) =>
        or(
          ...
          and(
            ...
            withEncryption
              ? or('api.*.encrypt_key', `api.${req.apiId}.encrypt_key`)
              : undefined,
          ),
        ),
      ),
    ...
    const toBeEncrypted = req.keys
      .filter((k) => k.plaintext)
      .map((k) => k.plaintext!)
            ...
            body: {
              ...
              ratelimit: k.ratelimit
                ? {
                    ...
                    duration:
                      k.ratelimit.duration ?? k.ratelimit.refillInterval,
                  }
                : undefined,
