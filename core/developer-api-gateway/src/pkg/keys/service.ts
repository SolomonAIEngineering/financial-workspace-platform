...
export class KeyService {
  ...
  > {
    try {
      ...
      c.set(
        'workspaceId',
        res.val.key?.forWorkspaceId ?? res.val.key?.workspaceId,
      )
  ...
  > {
    ...
    if (
      (data.forWorkspace && !data.forWorkspace.enabled) ||
     ...
     */
    const expires = data.key.expires
      ? new Date(data.key.expires).getTime()
      : undefined
    if (expires) {
      if (expires < Date.now()) {
        return Ok({
      ...
      if (!limited.valid) {
        return Ok({
          ...
          authorizedWorkspaceId:
            data.key.forWorkspaceId ?? data.key.workspaceId,
          permissions: data.permissions,
        })
      }
    }
  ...
  private async ratelimit(
    ...
    if (!this.rateLimiter) {
      this.logger.error(
        'ratelimiting is not enabled, but a key has ratelimiting enabled',
      )
      return [true, undefined]
    }
