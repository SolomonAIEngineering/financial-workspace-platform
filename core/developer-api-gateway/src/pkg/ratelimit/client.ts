...
export class AgentRatelimiter implements RateLimiter {
  ...
  public async limit(
    ...
    try {
      if (req.async) {
        ...
        const lookup =
          `RL_${req.limit}_${Math.round(req.interval / 1000)}s` as keyof typeof c.env
        const binding = c.env[lookup]

        if (binding) {
          const res = await cloudflareRatelimiter
            .parse(binding)
            .limit({ key: req.identifier })
  ...
  private async _limit(
    ...
    const isOlderThan60s = isolateCreatedAt
      ? Date.now() - isolateCreatedAt > 60_000
      : false
    const shouldSyncOnNoData =
      isOlderThan60s && Math.random() < c.env.SYNC_RATELIMIT_ON_NO_DATA
    ...
    try {
      ...
      for (let i = 0; i <= 3; i++) {
        try {
          ...
          this.logger.warn(
            'calling the agent for ratelimiting failed, retrying ...',
            {
              identifier: req.identifier,
      ...
      if (!res) {
        ...
        return Err(
          new RatelimitError({ message: err?.message ?? 'ratelimit failed' }),
        )
      }
