...
} from '@/routes/v1_apis_createApi'
import type {
  V1ApisDeleteApiRequest,
  V1ApisDeleteApiResponse,
} from '@/routes/v1_apis_deleteApi'
...
test(
  ...
  async (t) => {
    ...
    const createApiResponse = await h.post<
      V1ApisCreateApiRequest,
      V1ApisCreateApiResponse
    >({
    ...
    for (let i = 0; i < 5; i++) {
      const createKeyResponse = await h.post<
        V1KeysCreateKeyRequest,
        V1KeysCreateKeyResponse
      >({
    ...
    expect(
      deleteApi.status,
      `status mismatch - ${JSON.stringify(deleteApi)}`,
    ).toEqual(200)
  },
  { timeout: 30_000 },
)
...
test(
  ...
  async (t) => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (i % 2 === 0) {
        const deleteKeyResponse = await h.post<
