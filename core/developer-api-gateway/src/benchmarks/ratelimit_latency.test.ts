    ...
    describe.each([
      ...
      test(
        ...
        async (t) => {
          ...
          const { p50, p90, p99 } = aggregateLatencies(
            checks.map(({ latency }) => latency),
          )

          expect(p50, 'latency p50 is too high').toBeLessThanOrEqual(
            threshold.p50,
          )
          expect(p90, 'latency p90 is too high').toBeLessThanOrEqual(
            threshold.p90,
          )
          expect(p99, 'latency p99 is too high').toBeLessThanOrEqual(
            threshold.p99,
          )
        },
        {
  ...
  const results = await Promise.all(
    new Array(opts.keys).fill(null).map(async () => {
      ...
      const res = await fetch(
        `${h.env.PLANETFALL_URL}/api/check/${opts.region}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
